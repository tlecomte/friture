...
class __AudioBackend(QtCore.QObject):
	...
	def open_stream(self, device):
		...
		if device['name'].lower() == "default":
			self.logger.info("Default device detected, searching for a real input device")

			devices = sounddevice.query_devices()

			for d in devices:
				if d['max_input_channels'] > 0 and "default" not in d['name'].lower():
					device_index = d['index']
					self.logger.info(f"Using fallback device: {d['name']}")
					break
		...
		nchannels_max = min(device['max_input_channels'], 2)  # the number of channels that we record
	...
	def log_supported_input_formats(self, device):
		...
		dtypes = [np.float32, np.int16, np.int8]
		supported_formats = []

		# Extra safety: skip clearly unsafe device names
		...
		if any(k in name for k in unsafe_keywords):
			self.logger.info(f"Skipping unsafe device '{device['name']}'")
			return

		# Still skip no-input devices
		if device.get('max_input_channels', 0) <= 0:
			self.logger.info(f"Skipping device with no input channels '{device['name']}'")
			return
			...
			for dtype in dtypes:
				try:
					sounddevice.check_input_settings(
						device=device['index'],
						channels=min(device['max_input_channels'], 2),  # limit channels
						dtype=dtype,
						extra_settings=None,
						samplerate=samplerate
					)
					supported_formats.append(f"{samplerate} Hz, {np.dtype(dtype).name}")
				except Exception:
					pass

		try:
			api_name = sounddevice.query_hostapis(device['hostapi'])['name']
		except Exception:
			api_name = "unknown"

		self.logger.info(f"Supported formats for '{device['name']}' on '{api_name}': {supported_formats}")
